...
O código recebe por parâmetro os valores tamanho do cromossomo, tamanho da população, quantidade de melhores, probabilidade de mutação e quantidade de gerações
depois passa esses valores para o algoritimo genético para os cálculos, porém sem interface visual
"""
#variar : tamanho do cromossomo, tamanho da população, prob cruzamento, prob de mutação e quantidade de gerações
...
def gerarElementosBinarios(vetor_aleatorio, tamanho_cromossomo):
    ...
    for numero in vetor_aleatorio:
        # Converte o número para string binária com preenchimento à esquerda
...
def separarPiores(vetor_prob, vetor_binario, n):
  ...
  # Ordene pelo valor da porcentagem em ordem crescente
...
def substituirPioresPorFilhos(populacao, piores_elementos, filhos_melhores):
  # Verifique se o número de piores elementos é igual ao número de filhos dos melhores
  ...
  # Localize os índices dos piores elementos na população
  indices_piores = [populacao.index(elemento) for elemento in piores_elementos]

  # Substitua os piores elementos pelos filhos dos melhores
  for indice, filho in zip(indices_piores, filhos_melhores):
    populacao[indice] = filho

  # Retorne a população atual após a substituição
  return populacao
...
tamanho_cromossomo = int(input("Qual o tamanho do cromossomo? "))
tamanho_populacao = int(input("Qual o tamanho da população? "))
qtd_melhores = int(input("Qual a quantidade de melhores por geração? "))
prob_mutacao = int(input("Qual a probabilidade de mutação? "))
qtd_geracoes = int(input("Qual a quantidade de gerações? "))
...
#popBin = misturarNovosAntigos(sMelhores, s_filhos) # pop = 100 e filhos = 20 entao 100 - 20piores +20filhos
popDec = novosValoresDecimais(popBin)
aux = 0

# Fazendo o laço de gerações
while aux < qtd_geracoes:
    imagemFuncao = gerarImagem(popDec)
    probabRolet = gerarProbabilidades(imagemFuncao)
    sMelhores = separarMelhores(probabRolet, popBin,qtd_melhores)
    recomb = sortearCasais(qtd_melhores)
